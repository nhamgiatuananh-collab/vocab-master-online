...
class VocabApp {
  ...
  init() {
    if (!localStorage.getItem('vmp_force_reload_v2')) {
        localStorage.removeItem('vocabmaster_words_v9');
  ...
  loadData() {
    try {
      const saved = localStorage.getItem('vocabmaster_words_v9');
      if (saved) {
  ...
  saveData() {
    try {
      localStorage.setItem('vocabmaster_words_v9', JSON.stringify(this.words));
